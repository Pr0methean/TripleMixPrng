...
fn mix(w_lo: Simd64, x_in: Simd64, t: Simd64, w_hi: Simd64, i_hi: Simd64) -> (Simd64, Simd64) {
    ...
    let op13 = w_hi ^ op12;
    let op7 = (i_hi ^ op13) ^ t;
    let op11 = rotl(op12 ^ rotl((i_hi ^ FEISTEL_CONSTANT_3) ^ op14, 23), 35) ^ ((op7 + op13) ^ w_lo);
    let op15 = simd_swizzle!(op11, [1, 3, 0, 2]);
    let op6 = (i_hi ^ op11) ^ op15;
    let op11 = simd_swizzle!(op6, [3, 2, 1, 0]);
    let op11 = simd_mul(rotl(op11, 40), op11 + ((op6 >> 18) ^ (op15 ^ op7)));
    let out0 = simd_swizzle!(op11, [3, 2, 0, 1]) - rotl(op11, 33);
    let out1 = out0 - op15;
    (out0, out1)
}
